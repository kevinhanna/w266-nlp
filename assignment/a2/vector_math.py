...
def find_nn_cos(v, Wv, k=5):
    ...
    numerators = np.dot(Wv, v)
    denominators = np.dot(np.linalg.norm(Wv, axis=1), np.linalg.norm(v))
    results = numerators/denominators
    
    idx = (np.argsort(results)[-k:])[::-1]
